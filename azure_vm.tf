...
resource "azurerm_virtual_machine_extension" "jenkins_terraform" {
  ...
  settings = <<SETTINGS
  {
          "fileUris": ["https://raw.githubusercontent.com/pc-dok/terraform-jenkins-azure/master/jenkins-init.sh"],
...
terraform {
  backend "azurerm" {
    storage_account_name = "rg2storageuaclrs"
    container_name       = "scripts"
    key                  = "terraform.tfstate"
    access_key           = "obkX0KcPz+k8woXNkNOQiCrdKclbO52NnCE4ehagu6ss0Fb8Q5ugIJg0bwvH+yt34pBQ9GrJ9qOVCkG+RIPvpQ=="
